...
class _$ResponseWrapperImpl<T> implements _ResponseWrapper<T> {
  const _$ResponseWrapperImpl({this.status, required this.data});
  ...
  @override
  String toString() {
    return 'ResponseWrapper<$T>(status: $status, data: $data)';
  }
...
abstract class _ResponseWrapper<T> implements ResponseWrapper<T> {
  const factory _ResponseWrapper(
      {final String? status, required final T data}) = _$ResponseWrapperImpl<T>;
